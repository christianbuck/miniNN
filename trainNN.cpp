...
void evalOnDev(NN& nn, Corpus& dev) {
  nn.resetDevMetrics();
...
void trainNN(const size_t n_inputs, const size_t n_outputs, const size_t n_hidden, const size_t n_iters,
    const size_t corpus_size, const Corpus& c, const size_t speed, const string& modelprefix,
    const string& loadModel, double& learningrate, const double& momentumrate, 
    Corpus& dev, const double regfactor=0.0, const size_t batch_size=1,
  ...
  for (size_t iteration = 0; iteration < n_iters; ++iteration) {
    ...
    for (size_t idx = 0; idx < corpus_size; idx+=batch_size) {
      for (size_t batch_idx = 0; batch_idx < batch_size && idx+batch_idx < corpus_size; ++batch_idx) {
        Sentence e, f; // f=src, e=tgt
        c.getEntry(idx + batch_idx, e, f);
        nn.updateGradientSparse(f, e);
        n_examples += 1;
...
int main(int argc, char**argv) {
  ...
  void *argtable[] = { Asource, Atarget, Adev_source, Adev_target, Amodel, Aload,  Aiters, Ahidden, Awevery, Aweveryi, Aeevery, Ab, Areg, Alr, Am, Aspeed, Averb, Ashuffle, Ahelp, Aend };
  ...
  size_t n_inputs = c.sourceDim();
  size_t n_outputs = c.targetDim();

  const bool haveDevData = Adev_source->count >0 && Adev_target->count > 0;
  Corpus dev;
  if (haveDevData) {
    cerr << "loading dev corpus ..." << endl;
    dev = Corpus(string(Adev_source->filename[0]), string(Adev_target->filename[0]));
    n_inputs = max(c.sourceDim(), dev.sourceDim());
    n_outputs = max(c.targetDim(), dev.targetDim());
  }
